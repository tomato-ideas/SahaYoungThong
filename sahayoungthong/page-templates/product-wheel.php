<?php
/**
 * Template Name: Product wheel page
 *
 * @package WordPress
...
<script type="text/javascript">
    function pagination(page,type,lim){
        ...
        if(langjs == 'TH'){
            var trm = 'รุ่น';
            var f_text = 'ขนาดแม็กซ์';
            var s_text = 'รูน๊อต';
            var p_text = 'ออฟเซ็ต';
        }else{
            var trm = 'Model';
            var f_text = 'Wheel size';
            var s_text = 'Fitment';
            var p_text = 'Offset(ET)';
        }
        //alert(series);
        //alert(wheel);
            ...
            success:function(data){
                ...
                var lpm1 = '';
                 if(howtoshow == 'list'){
                  ...
                  show +=                   '<th>Alloy Wheel size</th>';
                  show +=                   '<th>Fitment</th>';
                  show +=                   '<th>Offset(ET)</th>';
                  show +=                   '<th>Picture</th>';
                  show +=               '</thead>';
                  show +=               '<tbody>'; 
                 }
...
<?php 
    ...
    $sqlpk =  "SELECT * FROM $tbl_name WHERE pd_type = $type" ;
                ...
                <div id="wrap_search">
                    <div id="product_box1_search_bt">
                        <select id="front_s" class="product_select" onchange="pagination(1,<?php echo $type; ?>)">
                            ...
                            $idx = get_the_ID();
                                ?> Alloy Wheel size <?php
                            }
                        } else {
                            ?> ขนาดล้อแม็กซ์ <?php
                        } ?></option>
                            <?php $anl = $wpdb->get_results("SELECT DISTINCT pd_front FROM wp_product WHERE pd_front != '-' AND pd_type = $type ORDER BY pd_front ASC");
                            foreach($anl as $yir){ ?>
                        ...
                        <select id="series_s" class="product_select" onchange="pagination(1,<?php echo $type; ?>)">
                            ...
                            <?php $anl2 = $wpdb->get_results("SELECT DISTINCT pd_series FROM wp_product WHERE pd_series != '-' AND pd_type = $type ORDER BY pd_series ASC");
                        ...
                        <select id="wheel_s" class="product_select" onchange="pagination(1,<?php echo $type; ?>)">
                           ...
                           <?php $anl3 = $wpdb->get_results("SELECT DISTINCT pd_pan_wheels FROM wp_product WHERE pd_pan_wheels != '-' AND pd_type = $type ORDER BY pd_pan_wheels ASC");
                    ...
                    <div id="product_box1_search_pic2" style="margin-left:-10px">
                        <img src="<?php echo $getdirect; ?>/images/MAX_Edit_2.jpg" style="cursor:pointer; width:190px; height:198;" onclick="pagination(1,<?php echo $type; ?>)">
                    </div>
                </div>
            </div>
...
<?php
    ...
    /* Get data. */
    $sql = "SELECT * FROM wp_product WHERE pd_type = $type LIMIT $start, $limit"; 
